...
{
    public class DLQQueueBatchConsumer
    {
        private readonly ILogger<DLQQueueBatchConsumer> _logger;
        const string _queueName = "simple-batch/$deadletterqueue";

        public DLQQueueBatchConsumer(
            ILogger<DLQQueueBatchConsumer> logger)
        {
            _logger = logger;
        }

        [Function(nameof(DLQQueueBatchConsumer))]
